...
class BubblesScene: SIFloatingCollectionScene {
    ...
    fileprivate func configure() {
        backgroundColor = SKColor(red: 0x21 / 0xff, green: 0x96 / 0xff, blue: 0xf3 / 0xff, alpha: 1.0)
    ...
    func sortedFloatingNodes() -> [SIFloatingNode]! {
        let sortedNodes = floatingNodes.sorted { (node: SIFloatingNode, nextNode: SIFloatingNode) -> Bool in
            let distance = distanceBetweenPoints(firstPoint: node.position, secondPoint: self.magneticField.position)
            let nextDistance = distanceBetweenPoints(firstPoint: nextNode.position, secondPoint: self.magneticField.position)
            return distance < nextDistance && node.state != SIFloatingNodeState.Selected
        }
        return sortedNodes
    }
    ...
    func actionForFloatingNode(_ node: SIFloatingNode!) -> SKAction {
        let action = SKAction.run({ () -> Void in
            if let index = self.floatingNodes.index(of: node) {
                self.removeFloatinNodeAtIndex(index: index)
                if node.state == SIFloatingNodeState.Selected {
                    self.throwNode(
                        node,
